...
public final class ProgramSession {
    ...
    public record RequestOutcome(boolean saved, boolean rolledBack, boolean cancelled) {}

    /** Saving failed after transaction completion; edits must not be replayed. */
    public static final class SaveFailure extends Exception {
        SaveFailure(Exception cause) { super(cause.getMessage(), cause); }
    }

    public ProgramSession(ScriptAccess script) {
        this.script = script;
        if (program() != null) program().addConsumer(consumer);
    }

    public Program program() { return script.program(); }
    ...
    public GhidraState state() { return script.state(); }
    ...
    public boolean disassemble(Address address) throws Exception { return script.disassemble(address); }
    public void analyzeAll() throws CancelledException {
    ...
    public void analyzeRange(AddressSetView range) throws CancelledException {
    ...
    public void analyzePending() throws CancelledException { analyzeChanges(null); }
    ...
    public void clearListing(Address start, Address end) throws Exception { script.clearListing(start, end); }

    /** Shared project-file traversal for program list and control snapshots. */
    public List<DomainFile> programFiles() throws CancelledException {
     ...
     */
    public <T> T preview(String description, Callable<T> operation) throws Exception {
    ...
    public RequestOutcome finishRequest(boolean successful) throws Exception {
        try {
    ...
    public boolean isCurrent(DomainFile domainFile) {
        return program() != null
            && program().getDomainFile().getPathname().equals(domainFile.getPathname());
    }

    public DomainFile findProgram(String name) {
    ...
    public void open(DomainFile domainFile) throws Exception {
    ...
    public void delete(DomainFile file) throws Exception {
        if (!isProgramFile(file)) {
            throw new IllegalArgumentException("Project file is not a program: " + file.getPathname());
        }
