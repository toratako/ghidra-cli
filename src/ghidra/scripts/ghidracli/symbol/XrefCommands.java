...
public final class XrefCommands {
    private final ProgramSession session;
    private final AddressResolver addressResolver;
    private final FunctionQueries functionQueries;

    public XrefCommands(ProgramSession session, AddressResolver addressResolver, FunctionQueries functionQueries) {
        this.session = session;
        this.addressResolver = addressResolver;
        this.functionQueries = functionQueries;
    }

    public JsonObject handleCreateMemory(JsonObject args) throws ghidra.util.exception.CancelledException {
    ...
    public JsonObject handleDelete(JsonObject args) throws ghidra.util.exception.CancelledException {
        Target target = mutationTarget(args);
    ...
    public JsonObject handleSetPrimary(JsonObject args) throws ghidra.util.exception.CancelledException {
        Target target = mutationTarget(args);
        SourceType expectedSource = source(args);
        ReferenceManager refs = session.program().getReferenceManager();
        Reference reference = refs.getReference(target.from, target.to, target.operand);
    ...
    public JsonObject handleXrefsTo(JsonObject args) throws ghidra.util.exception.CancelledException {
    ...
    public JsonObject handleXrefsFrom(JsonObject args) throws ghidra.util.exception.CancelledException {
