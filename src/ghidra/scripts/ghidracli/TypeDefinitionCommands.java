...
final class TypeDefinitionCommands {
    private final ProgramSession session;
    private final TypeResolver typeResolver;

    TypeDefinitionCommands(ProgramSession session, TypeResolver typeResolver) {
        this.session = session;
        this.typeResolver = typeResolver;
    }

    JsonObject handleClone(JsonObject args) {
    ...
    JsonObject handleMove(JsonObject args) {
        if (session.program() == null) return errorResult("No program loaded");
        try {
    ...
    JsonObject handleCategoryList(JsonObject args) {
        if (session.program() == null) return errorResult("No program loaded");
        try {
    ...
    JsonObject handleCategoryCreate(JsonObject args) {
        if (session.program() == null) return errorResult("No program loaded");
        try {
            CategoryPath path = categoryPath(getArgString(args, "path"));
            DataTypeManager dtm = session.program().getDataTypeManager();
    ...
    JsonObject handleCategoryDelete(JsonObject args) {
        if (session.program() == null) return errorResult("No program loaded");
        try {
            CategoryPath path = categoryPath(getArgString(args, "path"));
