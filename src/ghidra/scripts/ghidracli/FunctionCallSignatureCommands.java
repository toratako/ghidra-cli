...
final class FunctionCallSignatureCommands {
    ...
    FunctionCallSignatureCommands(ProgramSession session, FunctionQueries functions) {
    ...
    JsonObject handleSet(JsonObject args) {
        if (session.program() == null) return errorResult("No program loaded");
        try {
            Site site = resolveSite(args);
    ...
    JsonObject handleClear(JsonObject args) {
        if (session.program() == null) return errorResult("No program loaded");
        try {
            Site site = resolveSite(args);
            Saved previous = findSaved(site);
