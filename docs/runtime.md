...
| `GHIDRA_CLI_DECOMPILE_TIMEOUT` | Per-function native limit for `decompile`, `pcode function --high`, `function var list/get/set`, `function set-return-type`, `type uses --kind variable` and `type field uses`; unset or 0 = unbounded, maximum 2,147,483 seconds |
